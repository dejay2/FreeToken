...
void launch_vocabulary_tile(const Tensor& x, const Weight& weight, Tensor& out,
    ...
    // One reduction profile for every compact decode/MTP vocabulary projection.
...
constexpr auto vocabulary_launchers(std::index_sequence<I...>) {
    return std::array<W8Launch, sizeof...(I)>{
        &launch_vocabulary_tile<8 * (static_cast<int>(I) + 1)>...};
}

constexpr auto kVocabularyLaunchers = vocabulary_launchers(std::make_index_sequence<6>{});
