...
source-to-object transforms. Sections 1 through 12 define the mixed NVFP4 profile and shared DFlash2
suffix. Section 13 defines the descriptor-selected QUASAR profile, Section 14
...
The canonical mixed NVFP4 conversion entry point is:
...
the fixed official source in Section 10.1. Conversion also requires the pinned DFlash2 source
defined in Section 12 and appends its shared 66-tensor suffix after Vision. The complete profile
has 1,334 objects: 1,328 tensors and six resources. Its numeric counts are:
...
Existing published QUASAR artifacts without the suffix retain ordinary decode, MTP, and Vision.
Selecting DFlash2 requires the complete suffix; inactive DFlash2 weights are validated but not
uploaded. The suffix does not alter the base QUASAR quantization or its descriptor selection.
...
python3 -m tools.convert.qwen3_8_27b.convert_nvfp4 \
  ...
  --dflash2-model /path/to/Qwen3.8-27B-DFlash2 \
  --out out/quasar-dflash2/qwen3_8_27b_nvfp4.ninfer \
  --device cuda
```
