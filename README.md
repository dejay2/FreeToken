...
The fork includes a small local settings page at `http://127.0.0.1:2031`. It edits the startup settings and can start, stop, or restart the main server on port 2020 while remaining available if the server fails to boot. Every setting has a plain-language explanation with the numbers measured in `docs/research/` and chips for what it costs or buys (speed, accuracy, card memory, PC memory, drive space, start-up time); amounts get sliders, paths get a folder browser that marks model folders, and rarely-touched settings sit behind a "Show expert settings" switch. When PC memory runs short the governor parks layers on the card first (512 shared expert slots each, measured at about 3 tok/s per layer) and spills to the SSD only once 1,024 shared slots remain, since one SSD layer forces eager decode at 8-11 tok/s (measured 2026-09-07/08, `docs/research/long-context-gpu-fault-2026-09-08.md`). The page also keeps a crash watchdog: if the server was serving and stops answering without anyone pressing Stop, it is started again from the saved settings (at most three times an hour; `FREETOKEN_AUTO_RESTART` on the Server & advanced tab turns this off). A `Diagnostic mode` toggle on the same tab boots with `CUDA_LAUNCH_BLOCKING=1` and CUDA graphs off so a card fault is reported at the kernel that caused it (see `docs/research/long-context-gpu-fault-2026-09-08.md`). The page reads the chosen model folder's `config.json` (never the weights) and sizes itself from it: the longest-chat limit, the layers-on-card slider top, the expert-slot slider top and the memory-per-expert numbers follow the model, a model card at the top says what was read, and the measured speed figures are shown only for the model they were taken on. The Models tab can preview a Hugging Face model, check memory and drive space, and download it without overwriting an existing folder. Register it for the current Windows user with:
