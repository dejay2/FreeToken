        ...
        with safetensors.safe_open(file, framework="pt", device=str(device)) as f:
            for raw_name in f.keys():
                ...
                if vision is not None:
                    if include_vision:
                        yield from _vision_tensors(vision, require_dense_vision_weight(raw_name, f.get_tensor(raw_name)), vision_buf)
                    continue
    ...
    try:
        for file in tqdm(
            ...
            for raw_name in reader.names_in(file):
                ...
                if vision is not None:
                    if include_vision:
                        yield from _vision_tensors(vision, require_dense_vision_weight(raw_name, reader.get_tensor(raw_name)), vision_buf)
                    continue
                if raw_name.endswith(CT_SCALE_SUFFIXES):
                    continue  # consumed with their text weight_packed
                name = _rename(raw_name)
