...
class Gemma4Model(BaseOP):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        x = embed_input_ids(self.embed_tokens, input_ids, get_global_ctx().batch, image_token_id=self.image_token_id)
...
class Gemma4ForConditionalGeneration(Gemma4ForCausalLM):
    ...
    def encode_images(
        ...
        """Compatibility API for preprocessed offline images; flatten valid soft tokens."""
        return self.embed_vision.forward(self.vision_tower.forward(pixel_values, image_position_ids))
...
class Gemma4UnifiedForConditionalGeneration(Gemma4ForCausalLM):
    ...
    def encode_images(
        ...
        """Compatibility API for preprocessed unified images, in image/token order."""
        features = self.vision_embedder.forward(pixel_values, image_position_ids)
        embeddings = self.embed_vision.forward(features)
