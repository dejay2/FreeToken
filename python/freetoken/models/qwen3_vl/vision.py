...
class VisionAttention(BaseOP):
    def __init__(self, vc: VisionConfig, *, prefix: str = ""):
        ...
        self.proj = LinearOProj(
            vc.hidden_size, vc.hidden_size, has_bias=True,
        )
...
class VisionMLP(BaseOP):
    def __init__(self, vc: VisionConfig, *, prefix: str = ""):
        self.linear_fc1 = LinearColParallelMerged(
            vc.hidden_size, [vc.intermediate_size], has_bias=True,
        )
        self.linear_fc2 = LinearRowParallel(
            vc.intermediate_size, vc.hidden_size, has_bias=True,
        )
...
class VisionBlock(BaseOP):
    def __init__(self, vc: VisionConfig, *, prefix: str = ""):
        ...
        self.attn = VisionAttention(vc, prefix=f"{prefix}.attn")
        self.mlp = VisionMLP(vc, prefix=f"{prefix}.mlp")
...
class VisionPatchMerger(BaseOP):
    def __init__(
        ...
        *,
        ...
        self.linear_fc1 = LinearColParallelMerged(
            merged, [merged], has_bias=True,
        )
        self.linear_fc2 = LinearRowParallel(
            merged, vc.out_hidden_size, has_bias=True,
        )
        self._merged = merged
...
class Qwen3VLVisionModel(BaseOP):
    ...
    def __init__(self, vc: VisionConfig, *, prefix: str = "visual"):
        self.patch_embed = VisionConv3dPatchEmbed(vc)
        self.pos_embed = _EmbeddingParams(vc.num_position_embeddings, vc.hidden_size)
        self.blocks = OPList(
            [VisionBlock(vc, prefix=f"{prefix}.blocks.{i}") for i in range(vc.depth)]
        )
        self.merger = VisionPatchMerger(vc, prefix=f"{prefix}.merger")
        self.deepstack_merger_list = OPList(
            [
                VisionPatchMerger(
                    vc, use_postshuffle_norm=True, prefix=f"{prefix}.deepstack_merger_list.{k}"
                )
