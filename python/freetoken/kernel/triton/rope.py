...
def apply_section_mrope_with_cos_sin_cache_inplace(
    positions: torch.Tensor,
    query: torch.Tensor,
    key: torch.Tensor,
    head_size: int,
...
def apply_mrope_torch_fallback(
    ...
    for t, heads in ((query, query.shape[1] // head_size), (key, key.shape[1] // head_size)):
        ...
        # float() aliases float32 inputs: compute both halves before either write.
        out_lo = (lo * cos - hi * sin).to(v.dtype)
        out_hi = (hi * cos + lo * sin).to(v.dtype)
        v[..., :half] = out_lo
        v[..., half:rotary_dim] = out_hi


__all__ = ["apply_rope_with_cos_sin_cache_inplace", "apply_section_mrope_with_cos_sin_cache_inplace"]
