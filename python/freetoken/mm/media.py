    ...
    for ref in refs:
        ...
        try:
            if ref.get("kind") == "b64":
                ...
                out.append(base64.b64decode(payload, validate=True) if ";base64" in header.lower() else unquote_to_bytes(payload))
                ...
                async with httpx.AsyncClient(timeout=30.0, follow_redirects=False) as client:
                    for redirects in range(6):
                        _check_media_domain(data, config)
                        async with client.stream("GET", data) as resp:
                            if resp.is_redirect:
                                if redirects == 5:
                                    raise ValueError("image redirect limit exceeded (5)")
                                data = str(resp.url.join(resp.headers["location"]))
                                continue
                ...
                out.append(await asyncio.to_thread(_load_local_media, data, config))
            elif not urlsplit(data).scheme or re.match(r"^[A-Za-z]:[\\/]", data):
                out.append(await asyncio.to_thread(_load_local_media, Path(data).absolute().as_uri(), config))
            else:
                raise ValueError(
                    "unsupported image source (expect http(s)/file url or base64)"
                )
            if len(out[-1]) > _MAX_IMAGE_BYTES:
                raise ValueError(f"image exceeds {_MAX_IMAGE_BYTES} bytes")
