        ...
        try:
            for raw in images:
                with Image.open(io.BytesIO(raw)) as opened:
                    pils.append(opened.convert("RGB"))
            items = self.process(pils)
        finally:
            for pil in pils:
                pil.close()
        if len(items) != len(slots):
            raise ValueError(f"image processor returned {len(items)} items for {len(slots)} images")
        out: list[int] = []
        cursor = 0
...
def get_mm_processor(model_path: str, mm: MultimodalConfig | None = None) -> MMProcessor | None:
    ...
    try:
        ...
        return None
    # Keep the fork's Qwen4 picture opt-in authoritative in every process.
    if spec.module == "freetoken.models.qwen4_exp" or "FREETOKEN_LOAD_VISION" in os.environ:
