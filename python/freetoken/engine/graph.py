...
class GraphCaptureBuffer:
    ...
    def copy_from(self, batch: Batch) -> None:
        ...
        if rope_positions is None:
            self.rope_positions[:, _slice].copy_(
                batch.positions.to(torch.int64).expand(3, -1)
            )
        else:
            self.rope_positions[:, _slice].copy_(rope_positions)
        if self.mrope_positions is not None:
            positions = batch.mrope_positions
            if positions is None:
                positions = batch.positions.unsqueeze(0).expand(3, -1)
            self.mrope_positions[:, _slice].copy_(positions)
...
class GraphRunner:
    ...
    def can_use_cuda_graph(self, batch: Batch) -> bool:
        # A DISK layer's per-step disk gather needs a host sync (topk_ids D2H) that a captured
        # graph cannot contain; the engine also rebuilds this runner with no graphs while any
        # layer is DISK, so this is the belt to that suspender. One bool read per batch.
