...
@pytest.mark.parametrize("name,dtype", [("visual.blocks.0.attn.qkv.weight", torch.uint8), ("visual.blocks.0.attn.qkv.weight_scale", torch.float32), ("visual.blocks.0.attn.qkv.weight", torch.float8_e4m3fn)])
def test_quantized_vision_is_rejected(name, dtype):
    from freetoken.models.qwen3_vl.weight import validate_vision_tensor

    with pytest.raises(NotImplementedError, match="Quantized vision"):
        validate_vision_tensor(name, torch.empty(2, 2, dtype=dtype))


def test_dense_vision_is_accepted():
    from freetoken.models.qwen3_vl.weight import validate_vision_tensor

    validate_vision_tensor("visual.blocks.0.attn.qkv.weight", torch.zeros(2, 2, dtype=torch.bfloat16))
...
def test_vision_tower_constructs_with_legacy_linear_layers():
    from freetoken.distributed import set_tp_info, try_get_tp_info
    from freetoken.models.qwen3_vl import Qwen3VLVisionModel

    if try_get_tp_info() is None:
        set_tp_info(rank=0, size=1)
    config = parse_config(_hf_config()).vision_config
    with torch.device("meta"):
        tower = Qwen3VLVisionModel(config)
    assert tower.state_dict()["blocks.0.attn.qkv.weight"].shape == (3 * config.hidden_size, config.hidden_size)


def test_qwen35_vision_reader_preserves_text_reader_and_rejects_quantized(tmp_path, monkeypatch):
    from safetensors.torch import save_file
    from freetoken.distributed import set_tp_info, try_get_tp_info
    from freetoken.models.qwen3_5_moe import weight
    from freetoken.models.qwen3_vl import weight as vision_weight

    if try_get_tp_info() is None:
        set_tp_info(rank=0, size=1)
    shard = tmp_path / "model.safetensors"
    monkeypatch.setattr(weight, "_iter_text_weights", lambda *a, **kw: iter([("model.norm.weight", torch.ones(4))]))
    monkeypatch.setattr(vision_weight, "iter_weight_files", lambda path: [str(shard)])
    save_file({"model.visual.blocks.0.attn.qkv.weight": torch.ones(12, 4)}, shard)
    result = dict(weight.iter_weights(str(tmp_path), torch.device("cpu"), include_moe_experts=False, include_non_moe=True))
    assert set(result) == {"model.norm.weight", "visual.blocks.0.attn.qkv.weight"}
    save_file({"model.visual.blocks.0.attn.qkv.weight": torch.zeros(12, 2, dtype=torch.uint8)}, shard)
    kwargs = dict(include_moe_experts=False, include_non_moe=True)
    with pytest.raises(NotImplementedError, match="Quantized vision"):
        list(weight.iter_weights(str(tmp_path), torch.device("cpu"), **kwargs))
    assert list(dict(weight.iter_weights(str(tmp_path), torch.device("cpu"), include_vision=False, **kwargs))) == ["model.norm.weight"]


def test_legacy_precomputed_embeddings_derive_placeholder_rows():
    table = torch.arange(24, dtype=torch.float32).reshape(6, 4)
    embed = SimpleNamespace(forward=lambda ids: table[ids], num_embeddings=6)
    features = torch.tensor([[10., 20., 30., 40.], [11., 21., 31., 41.]])
    batch = SimpleNamespace(mm_embeds=features)
    actual = embed_input_ids(embed, torch.tensor([1, 5, 2, 5]), batch, image_token_id=5)
    torch.testing.assert_close(actual, torch.stack([table[1], features[0], table[2], features[1]]))
    with pytest.raises(ValueError, match="rows"):
        embed_input_ids(embed, torch.tensor([1, 5]), batch, image_token_id=5)
