...
def test_qwen4_processor_respects_legacy_picture_opt_in(monkeypatch):
    from freetoken.mm.processor import get_mm_processor
    import freetoken.utils

    config = _hf_config()
    config.architectures = ["Qwen4ExpForConditionalGeneration"]
    monkeypatch.setattr(freetoken.utils, "cached_load_hf_config", lambda _: config)
    monkeypatch.delenv("FREETOKEN_LOAD_VISION", raising=False)
    assert get_mm_processor("/unused") is None
    monkeypatch.setenv("FREETOKEN_LOAD_VISION", "1")
    assert isinstance(get_mm_processor("/unused"), QwenVLMMProcessor)
    assert get_mm_processor("/unused", MultimodalConfig(disabled_encoders=frozenset({"vision"}))) is None


def test_explicit_legacy_picture_disable_applies_to_other_families(monkeypatch):
    from freetoken.mm.processor import get_mm_processor
    import freetoken.utils

    monkeypatch.setattr(freetoken.utils, "cached_load_hf_config", lambda _: _hf_config())
    monkeypatch.delenv("FREETOKEN_LOAD_VISION", raising=False)
    assert isinstance(get_mm_processor("/unused"), QwenVLMMProcessor)
