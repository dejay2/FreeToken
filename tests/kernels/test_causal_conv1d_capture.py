...
def test_mtp_verify_graph_binding_declares_its_fixed_row_width():
    from freetoken.engine.mtp_fast_verify import _MTPVerifyGraphBuffer

    buffer = _MTPVerifyGraphBuffer.init(3, 6, torch.device("cpu"))
    batch = _mtp_verify_batch(3)
    buffer.copy_from(batch)
    buffer.bind(batch)

    assert batch.fla_metadata.max_seq_len == 3


def _mtp_verify_batch(width: int):
    from freetoken.core import Batch, Req, SamplingParams
    ...
    req.linear_slot_idx = 300
    batch = Batch(reqs=[req], phase="prefill")
    batch.padded_reqs = batch.reqs
    batch.input_ids = torch.arange(width, dtype=torch.int32)
    batch.positions = torch.arange(width, dtype=torch.int32)
    batch.out_loc = torch.arange(width, dtype=torch.int32)
    batch.rope_positions = batch.positions.to(torch.int64).expand(3, -1).clone()
    batch.linear_table_idx = torch.tensor([300], dtype=torch.int32)
